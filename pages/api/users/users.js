...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    try {
      const { first_name, last_name, email, phone_number, country_code } = req.body;
      if (!first_name || !last_name || !email || !phone_number || !country_code) {
        return sendResponse(res, false, {}, 'Missing required fields', 400, 400);
      }
      const newUser = await UserModel.createUser(first_name, last_name, email, phone_number, country_code);
